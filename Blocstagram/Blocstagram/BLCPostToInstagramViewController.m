...
- (FilterViewCell*) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    FilterViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"cell" forIndexPath:indexPath];
    
    UICollectionViewFlowLayout *flowLayout = (UICollectionViewFlowLayout *)self.filterCollectionView.collectionViewLayout;
    CGFloat thumbnailEdgeSize = flowLayout.itemSize.width;
    
    cell.thumbnail.frame = CGRectMake(0, 0, thumbnailEdgeSize, thumbnailEdgeSize);
    cell.label.frame = CGRectMake(0, thumbnailEdgeSize, thumbnailEdgeSize, 20);
...
- (void) addFiltersToQueue {
    ...
    // Rollercoaster filter
    
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *edgeFilter = [CIFilter filterWithName:@"CIColorPosterize"];
        CIFilter *hatchFilter = [CIFilter filterWithName:@"CIHatchedScreen"];
        
        if (edgeFilter) {
            [edgeFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            
            CIImage *result = edgeFilter.outputImage;
            
            if (hatchFilter) {
                [hatchFilter setValue:result forKeyPath:kCIInputImageKey];
                result = hatchFilter.outputImage;
            }
            
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Rollercoaster", @"Rollercoaster Filter")];
        }
    }];
